...
#define LOG_TAG "android.hardware.bluetooth@1.0-intf-universal5420"
...
void sco_audiostate_cb(bt_vendor_op_result_t result) {
  ...
  ALOGD("%s result: %d -> sco_audiostate_cb(status: %d)", __func__, result, status);
}
...
/**** Make this function static to set sco_audiostate_cb in runtime to work ***/
...
void VendorInterface::Shutdown() {
  ...
  ALOGI("%s: Shutdown complete", __func__);
}
...
void VendorInterface::Close() {
  ...
  ALOGI("%s: Interface closed", __func__);
}
...
size_t VendorInterface::Send(uint8_t type, const uint8_t* data, size_t length) {
  ...
  if (lpm_wake_deasserted == true) {
    ...
    ALOGV("%s: Sent (%02x)", __func__, data[0] | (data[1] << 8));
...
void VendorInterface::SetAudioState(void *param) {
    ALOGI("%s:", __func__);
  LOG_ALWAYS_FATAL_IF(!g_vendor_interface, "%s: No Vendor interface!", __func__);
  g_vendor_interface->ScoAudioSet(param);  
}
